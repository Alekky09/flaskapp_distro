...
class LoginForm(FlaskForm):
    username = StringField("Username", validators=[InputRequired(), Length(min=4, max=15), Regexp('\w+$', message="Username must contain only letters numbers or underscore!")])
    password = PasswordField("Password", validators=[InputRequired(), Length(min=8, max=80)])
    remember = BooleanField("Remember me")

class RegisterForm(FlaskForm):
    username = StringField("Username", validators=[InputRequired(), Length(min=4, max=15, message="Username must be between 4 and 15 characters long."), Regexp('\w+$', message="Username must contain only letters numbers or underscore!")])
